...
class DB_CRUD_ops(object):
    ...
    def get_stock_info(self, stock_symbol):
        ...
        try:
            ...
            substituted_query = query.replace('?', "'" + stock_symbol + "'")
            res += "[QUERY] " + substituted_query + "\n"
            ...
            has_restricted_char = any([char in substituted_query for char in restricted_chars])
    ...
    def update_stock_price(self, stock_symbol, price):
        ...
        try:
            ...
            res += "[QUERY] " + query.replace('?', "'" + str(price) + "'", 1).replace('?', "'" + stock_symbol + "'", 1) + "\n"
